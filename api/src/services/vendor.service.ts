...
const STUB_VENDORS: VendorListItemDto[] = [
  ...
  { vendorId: 'vendor-003', vendorName: 'Gamma Ops', country: 'FR', regionScope: 'EU', category: 'orchestration', trackingStatus: 'review_queue', marketMaturityScore: 64, integrationScore: 81, governanceScore: 74, overallScore: 73, confidence: 71, freshnessStatus: 'failed', asOfDate: '2026-04-08', sourceRunId: 'run-2026-04-11-001', deltaStatus: 'downgraded', openEscalation: true },
  { vendorId: 'vendor-004', vendorName: 'Delta Vertical', country: 'NL', regionScope: 'EU', category: 'vertical_solution', trackingStatus: 'active', marketMaturityScore: 79, integrationScore: 67, governanceScore: 83, overallScore: 76, confidence: 84, freshnessStatus: 'fresh', asOfDate: '2026-04-10', sourceRunId: 'run-2026-04-11-001', deltaStatus: 'upgraded', openEscalation: false },
  { vendorId: 'vendor-005', vendorName: 'Epsilon Cloud', country: 'SE', regionScope: 'EU', category: 'other', trackingStatus: 'inactive', marketMaturityScore: 58, integrationScore: 55, governanceScore: 61, overallScore: 58, confidence: 63, freshnessStatus: 'unknown', asOfDate: '2026-04-01', sourceRunId: 'run-2026-04-11-001', deltaStatus: 'new', openEscalation: false },
];

const STUB_VENDOR_DETAILS: Record<string, VendorDetailDto> = {
  'vendor-001': {
    vendorId: 'vendor-001', vendorName: 'Alpha AI', legalEntityName: 'Alpha AI GmbH', websiteUrl: 'https://alpha.example.com', headquartersCountry: 'DE', euPresence: true,
    category: 'platform', shortDescription: 'Enterprise AI platform.', trackingStatus: 'active', reviewQueueReason: null,
    marketMaturityScore: 82, integrationScore: 77, governanceScore: 80, overallScore: 80, confidence: 88, confidenceReason: 'Strong public signal coverage.', scoringRubricVersion: 'v1.0.0', secondOpinionModel: 'gpt-4.1-mini', scoreDivergencePct: 6.5,
    asOfDate: '2026-04-11', sourceRunId: 'run-2026-04-11-001', freshnessStatus: 'fresh', latestRunId: 'run-2026-04-11-001', latestManifestRef: 'latest.manifest.json', snapshotPath: '/snapshots/2026-04-11/vendor-001.json',
    euHostingClaim: true, dataResidencyEu: 'yes', identityIntegration: 'sso_scim', ssoSupport: true, auditLoggingClaim: true, complianceClaims: ['ISO27001', 'SOC2'], securityDisclosures: 'Public trust center available.', humanReviewRequired: false,
    sourceCount: 14, latestEvidenceCount: 8, primarySources: ['vendor docs', 'trust center', 'pricing page'], sourceQualityFlag: 'high',
  },
};

const freshnessOrder: Record<VendorListItemDto['freshnessStatus'], number> = { fresh: 4, stale: 3, unknown: 2, failed: 1 };

/**
 * WP-02.2/WP-02.3: typed stub dataset until pipeline/read-model contracts are integrated.
 */
export class VendorService {
  public async listVendors(query: VendorListQueryDto): Promise<VendorListResponseDto> {
    ...
    if (query.trackingStatus) items = items.filter((item) => item.trackingStatus === query.trackingStatus);
    if (query.freshnessStatus) items = items.filter((item) => item.freshnessStatus === query.freshnessStatus);
    if (typeof query.openEscalation === 'boolean') items = items.filter((item) => item.openEscalation === query.openEscalation);
